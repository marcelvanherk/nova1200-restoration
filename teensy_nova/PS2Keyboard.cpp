/*
  ...
  ** Modified for easy interrup pin assignement on method begin(datapin,irq_pin). Cuningan <cuninganreset@gmail.com> **
  ...
  // mvh 20200906: added uses_ctrl, US keymap adds ctrl keys, CAPS_LOCK (unset with shift)
  // mvh 20200906: Added 50 us delay in interrupt routine - fixes issues on teensy_nova
*/
...
const PROGMEM PS2Keymap_t PS2Keymap_US = {
  ...
  // with ctrl (used)
	{0, PS2_F9, 0, PS2_F5, PS2_F3, PS2_F1, PS2_F2, PS2_F12,
	0, PS2_F10, PS2_F8, PS2_F6, PS2_F4, PS2_TAB, '`', 0,
	0, 0 /*Lalt*/, 0 /*Lshift*/, 0, 0 /*Lctrl*/, 'q'&31, '1'&31, 0,
	0, 0, 'z'&31, 's'&31, 'a'&31, 'w'&31, '2'&31, 0,
	0, 'c'&31, 'x'&31, 'd'&31, 'e'&31, '4'&31, '3'&31, 0,
	0, ' '&31, 'v'&31, 'f'&31, 't'&31, 'r'&31, '5'&31, 0,
	0, 'n'&31, 'b'&31, 'h'&31, 'g'&31, 'y'&31, '6'&31, 0,
	0, 0, 'm'&31, 'j'&31, 'u'&31, '7'&31, '8'&31, 0,
	0, ','&31, 'k'&31, 'i'&31, 'o'&31, '0'&31, '9'&31, 0,
	0, '.'&31, '/'&31, 'l'&31, ';'&31, 'p'&31, '-'&31, 0,
	0, 0, '\''&31, 0, '['&31, '='&31, 0, 0,
	0 /*CapsLock*/, 0 /*Rshift*/, PS2_ENTER /*Enter*/, ']'&31, 0, '\\'&31, 0, 0,
	0, 0, 0, 0, 0, 0, PS2_BACKSPACE, 0,
	0, '1', 0, '4', '7', 0, 0, 0,
	'0', '.', '2', '5', '6', '8', PS2_ESC, 0 /*NumLock*/,
	PS2_F11, '+', '3', '-', '*', '9', PS2_SCROLL, 0,
...
static char get_iso8859_code(void)
{
	...
	while (1) {
		...
		if (s == 0xF0) {
			...
			if (state & BREAK) {
				if (s == 0x12) {
				state &= ~(SHIFT_L|CAPS_LOCK);
				} else if (s == 0x59) {
					state &= ~(SHIFT_R|CAPS_LOCK);
				} else if (s == 0x14) {
					state &= ~CTRL_L;
			...
			} else if ((state & (CTRL_L | CTRL_R)) && keymap->uses_ctrl) {
				if (s < PS2_KEYMAP_SIZE)
					c = pgm_read_byte(keymap->ctrl + s);
			} else if (state & (SHIFT_L | SHIFT_R)) {
				if (s < PS2_KEYMAP_SIZE)
					c = pgm_read_byte(keymap->shift + s);
			} else if ((state & CAPS_LOCK) && s<PS2_KEYMAP_SIZE && pgm_read_byte(keymap->noshift + s)>='a' && pgm_read_byte(keymap->noshift + s)<='z') {
				if (s < PS2_KEYMAP_SIZE)
					c = pgm_read_byte(keymap->shift + s);
			} else {
				if (s < PS2_KEYMAP_SIZE)
